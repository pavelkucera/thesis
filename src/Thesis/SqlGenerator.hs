...
generateSql (Query _ (Select sAgg sFrom sWhere)) = emit "SELECT " <>
...
emitExpr (PrefixOp identifier expr) = emit identifier <>
                                      emitExpr expr
emitExpr (PostfixOp identifier expr) = emitExpr expr <>
                                       emit identifier
emitExpr (BinaryOp expr1 identifier expr2) = emitExpr expr1 <>
                                             emit identifier <>
                                             emitExpr expr2
emitExpr (FunctionCall identifier exprs) = emit identifier <>
                                           emit "(" <>
                                           mconcat (emitExpr `map` exprs) <>
                                           emit ")"

emitAggregation :: Aggregation -> SqlPart
emitAggregation (Average expr) = emit "AVG(" <>
                                 emitExpr expr <>
                                 emit ") "
emitAggregation (Sum expr) = emit "SUM(" <>
                             emitExpr expr <>
                             emit ") "
...
emitWhere (Just expr) = emit "WHERE " <>
                        emitExpr expr

emitFrom :: Identifier -> SqlPart
emitFrom identifier = emit "FROM " <>
